    ...
    <div class="h-screen flex flex-col overflow-hidden">
      ...
      <div class="flex-1 min-w-0 relative overflow-hidden">
        ...
        {:else}
          ...
          {/if}
        {/if}
        <!-- Properties panel as floating bottom sheet when element selected -->
        {#if $selectedElementId || $selectedRoomId}
          <div class="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-xl max-h-[45vh] overflow-y-auto overscroll-contain rounded-t-xl z-30">
            <div class="flex items-center justify-between px-4 py-2 border-b border-gray-100">
              <span class="text-sm font-semibold text-gray-700">Properties</span>
              <button
                onclick={() => { selectedElementId.set(null); selectedRoomId.set(null); }}
                class="text-gray-400 hover:text-gray-600 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-full hover:bg-gray-100"
      ...
      <MobileBottomBar onOpenDrawer={() => drawerOpen = true} />
    </div>

    <!-- Build Panel as bottom drawer -->
    <MobileDrawer bind:open={drawerOpen} title="Build & Objects" heightClass="max-h-[80vh]">
      {#snippet children()}
        <BuildPanel />
      {/snippet}
    </MobileDrawer>
