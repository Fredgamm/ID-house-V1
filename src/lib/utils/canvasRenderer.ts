...
export function drawDoorOnWall(cs: CanvasState, wall: Wall, door: Door): void {
  ...
  drawOpeningGapOnWall(cs, wall, t, door.width);
...
export function drawOpeningGapOnWall(cs: CanvasState, wall: Wall, position: number, width: number): void {
  const { ctx, zoom } = cs;
  const wpt = wallPointAt(wall, position);
  const s = wts(cs, wpt.x, wpt.y);
  const tan = wallTangentAt(wall, position);
  const ux = tan.x, uy = tan.y;
  const nx = -uy, ny = ux;
  const halfOpening = (width / 2) * zoom;
  const thickness = wallThicknessScreen(wall, zoom);

  ctx.fillStyle = '#fafafa';
  const gux = ux * halfOpening;
  const guy = uy * halfOpening;
