<script lang="ts">
  ...
  import { getOpeningsForWall, type RenderOpening } from '$lib/utils/wallOpenings';
  ...
  function buildWalls(floor: Floor) {
    ...
    for (const wall of floor.walls) {
      ...
      const doorOpenings = getOpeningsForWall(wall, floor.walls, floor.doors);
      const winOpenings = getOpeningsForWall(wall, floor.walls, floor.windows);
      ...
      const doorOpeningsForBB = doorOpenings;
  ...
  function buildWallSegments(
    ...
    doors: RenderOpening<Door>[], windows: RenderOpening<Win>[]
  ): WallSegment[] {
    type Opening = { pos: number; width: number; bottomY: number; topY: number };
    const openings: Opening[] = [];
    for (const d of doors) {
      openings.push({ pos: d.position * wallLen, width: d.width, bottomY: 0, topY: d.opening.height || 210 });
    }
    for (const w of windows) {
      openings.push({ pos: w.position * wallLen, width: w.width, bottomY: w.opening.sillHeight, topY: w.opening.sillHeight + w.opening.height });
    }
