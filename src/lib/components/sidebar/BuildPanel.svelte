<script lang="ts">
  import { selectedTool, placingFurnitureId, placingDoorType, placingWindowType, placingStair, addStair, placingColumn, placingColumnShape, activeFloor, setBackgroundImage, canvasCamX, canvasCamY, placingRoomPresetId, placingRoomTemplateName } from '$lib/stores/project';
  ...
  let currentPlacingRoomId = $state<string | null>(null);
  placingRoomPresetId.subscribe(id => { currentPlacingRoomId = id; });

  function onPresetClick(presetId: string, templateName?: string) {
    // Activate ghost preview mode — canvas will place on click
    placingRoomPresetId.set(presetId);
    placingRoomTemplateName.set(templateName ?? null);
    // Clear any active furniture placement
    placingFurnitureId.set(null);
