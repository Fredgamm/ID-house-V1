<script lang="ts">
  ...
  import { getOpeningsForWall } from '$lib/utils/wallOpenings';
  ...
  let currentPlacingRoomPresetId: string | null = $state(null);
  let currentPlacingRoomTemplateName: string | null = $state(null);
  ...
  function drawOpeningGapOnWall(wall: Wall, position: number, openingWidth: number) {
    _drawOpeningGapOnWall(getCS(), wall, position, openingWidth);
  }
  ...
  function drawRoomPreview() {
    if (!currentPlacingRoomPresetId) return;
    const preset = roomPresets.find(p => p.id === currentPlacingRoomPresetId);
    if (!preset) return;
    const walls = preset.getWalls(400, 300);
    // Compute bounding box center (same as placePreset)
    let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    ...
    const ox = mousePos.x - cx;
    const oy = mousePos.y - cy;
    ...
    ctx.beginPath();
    const first = walls[0];
    const fs = worldToScreen(ox + first.start.x, oy + first.start.y);
    ctx.moveTo(fs.x, fs.y);
    for (const w of walls) {
      const e = worldToScreen(ox + w.end.x, oy + w.end.y);
      ctx.lineTo(e.x, e.y);
    }
    ctx.closePath();
    ctx.fill();
    ...
    // Label
    const center = worldToScreen(mousePos.x, mousePos.y);
    const label = currentPlacingRoomTemplateName ?? preset.name;
    ctx.globalAlpha = 0.9;
    ctx.fillStyle = '#1e40af';
    ctx.font = `bold ${Math.max(11, 13 / zoom * zoom)}px sans-serif`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(label, center.x, center.y - 8);
    ctx.font = `${Math.max(9, 11 / zoom * zoom)}px sans-serif`;
    ctx.fillStyle = '#3b82f6';
    ctx.fillText('Click to place · ESC to cancel', center.x, center.y + 10);
    ctx.restore();
  }
  ...
  function draw() {
    ...
    if (layerVis.walls) {
      if (showDoors) {
        for (const wall of floor.walls) {
          for (const opening of getOpeningsForWall(wall, floor.walls, floor.doors)) {
            if (opening.projected) drawOpeningGapOnWall(wall, opening.position, opening.width);
          }
        }
      }
      if (showWindows) {
        for (const wall of floor.walls) {
          for (const opening of getOpeningsForWall(wall, floor.walls, floor.windows)) {
            if (opening.projected) drawOpeningGapOnWall(wall, opening.position, opening.width);
          }
        }
      }
    ...
    // Room ghost preview
    if (currentPlacingRoomPresetId) drawRoomPreview();
  ...
  onMount(() => {
    ...
    const unsub_roomPreset = placingRoomPresetId.subscribe((id) => { currentPlacingRoomPresetId = id; markDirty(); });
    const unsub_roomTmpl = placingRoomTemplateName.subscribe((n) => { currentPlacingRoomTemplateName = n; markDirty(); });
    ...
    return () => { resizeObs.disconnect(); unsub1(); unsub2(); unsub3(); unsub4(); unsub5(); unsub6(); unsub7(); unsub8(); unsub9(); unsub10(); unsub11(); unsub12(); unsub13(); unsub_multi(); unsub14(); unsub_col(); unsub_cols(); unsub_layers(); unsub_snapgrid(); unsub_roomPreset(); unsub_roomTmpl(); document.removeEventListener('paste', handlePaste); };
  });
  ...
  function onMouseDown(e: MouseEvent) {
    ...
    // Room ghost placement
    if (currentPlacingRoomPresetId) {
      const preset = roomPresets.find(p => p.id === currentPlacingRoomPresetId);
      if (preset) {
        const pos = { x: snap(wp.x), y: snap(wp.y) };
        const template = currentPlacingRoomTemplateName
          ? roomTemplates.find(t => t.name === currentPlacingRoomTemplateName) ?? null
          : null;
        placeRoomTemplate(preset, pos, template);
      }
      placingRoomPresetId.set(null);
      placingRoomTemplateName.set(null);
      return;
    }
  ...
  let cursorStyle = $derived(
    ...
    currentPlacingRoomPresetId ? 'copy' :
    'crosshair'
  );
</script>
