<script lang="ts">
  ...
  function getActiveFloorName() {
    return floors.find((floor) => floor.id === activeFloorId)?.name ?? 'Floors';
  }
  ...
  onMount(() => {
    ...
    function handleClickOutside(e: MouseEvent) {
      ...
      if (floorOpen && floorRef && !floorRef.contains(e.target as Node)) {
        floorOpen = false;
      }
    }
    function handleKeydown(e: KeyboardEvent) {
      if (exportOpen) exportOpen = false;
      if (floorOpen) floorOpen = false;
...
<div class="h-12 bg-gradient-to-r from-slate-800 to-slate-700 flex items-center px-4 gap-3 shrink-0 shadow-sm">
  ...
  <!-- Floor list -->
  <div class="relative" bind:this={floorRef}>
    <button
      class="group flex h-11 min-w-[54px] flex-col items-center justify-center gap-0.5 rounded-lg px-2 text-white/80 transition-colors hover:bg-white/10 hover:text-white {floorOpen ? 'bg-white/15 text-white' : ''}"
      onclick={() => floorOpen = !floorOpen}
      title={`Floors: ${getActiveFloorName()}`}
      aria-label="Open floor list"
      aria-haspopup="menu"
      aria-expanded={floorOpen}
    >
      <span class="flex h-6 w-8 items-center justify-center rounded-md bg-white/90 text-slate-800 shadow-sm transition-colors group-hover:bg-white">
      ...
      <span class="max-w-[48px] truncate text-[10px] font-medium leading-none">Floors</span>
    </button>

    {#if floorOpen}
      <div class="absolute left-0 top-full z-50 mt-2 w-64 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-xl" role="menu" aria-label="Floor list">
        <div class="flex items-center justify-between border-b border-gray-100 px-3 py-2">
          <div>
            <div class="text-xs font-semibold uppercase tracking-wide text-gray-400">Floors</div>
            <div class="text-sm font-semibold text-slate-800">{getActiveFloorName()}</div>
          </div>
          <span class="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-500">{floors.length}</span>
        </div>
        ...
        <div class="max-h-64 overflow-y-auto py-1">
          {#each floors as fl}
            <div class="flex items-center gap-1 px-1">
              <button
                class="flex min-h-[44px] flex-1 items-center gap-3 rounded-md px-2.5 py-2 text-left text-sm transition-colors {fl.id === activeFloorId ? 'bg-blue-50 text-slate-900' : 'text-gray-700 hover:bg-gray-50'}"
                onclick={() => { setActiveFloor(fl.id); floorOpen = false; }}
                role="menuitem"
              >
                <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-md {fl.id === activeFloorId ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}">
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M12 2 2 7l10 5 10-5-10-5Z"/>
                    <path d="m2 12 10 5 10-5"/>
                  </svg>
                ...
                <span class="min-w-0 flex-1">
                  <span class="block truncate font-medium">{fl.name}</span>
                  <span class="block text-xs text-gray-400">{fl.id === activeFloorId ? 'Active floor' : 'Switch to floor'}</span>
                </span>
              </button>

              <button
                class="flex h-9 w-9 shrink-0 items-center justify-center rounded-md text-gray-300 transition-colors hover:bg-red-50 hover:text-red-500 disabled:pointer-events-none disabled:opacity-30"
        ...
        <button
          class="flex min-h-[44px] w-full items-center justify-center gap-2 border-t border-gray-100 px-3 py-2 text-sm font-medium text-blue-600 transition-colors hover:bg-blue-50"
          onclick={() => { onAddFloor(); floorOpen = false; }}
        >
